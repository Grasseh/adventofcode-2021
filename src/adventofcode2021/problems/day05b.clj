(ns adventofcode2021.problems.day05b)
...
; From 6,4 to 2,0 -> 2,0 3,1 4,2 5,3 and 6,4
(defn count-big-d [count-map vent]
  ; (let [diff (- (min (:x1 vent) (:x2 vent)) (min (:y1 vent) (:y2 vent)))]
  ; (reduce
  ;   #(increment-map %1 %2 (+ %2 diff))
  ;   count-map
  ;   (range (min (:x1 vent) (:x2 vent)) (+ (max (:x1 vent) (:x2 vent)) 1))
  ; )))
  (reduce
    #(
      increment-map
      %1
      (+ %2 (min (:x1 vent) (:x2 vent)))
      (+ %2 (min (:y1 vent) (:y2 vent)))
    )
    count-map
    (range 0 (+ (Math/abs (- (:x1 vent) (:x2 vent))) 1))
  ))

; From 5,5 to 8,2 -> 5,5 6,4 7,3 and 8,2
(defn count-small-d [count-map vent]
  (reduce
    #(
      increment-map
      %1
      (+ %2 (min (:x1 vent) (:x2 vent)))
      (- (max (:y1 vent) (:y2 vent)) %2)
    )
    count-map
    (range 0 (+ (Math/abs (- (:x1 vent) (:x2 vent))) 1))
  ))

(defn count-diagonal-map [count-map vent]
  (if
    (or
      (and (> (:x1 vent) (:x2 vent)) (> (:y1 vent) (:y2 vent)))
      (and (< (:x1 vent) (:x2 vent)) (< (:y1 vent) (:y2 vent)))
    )
    (count-big-d count-map vent)
    (count-small-d count-map vent)
  ))
...
(defn count-vent [count-map vent]
  (cond
    ...
    :else (count-diagonal-map count-map vent)
  ))
