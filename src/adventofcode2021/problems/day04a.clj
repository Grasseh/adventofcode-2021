(ns adventofcode2021.problems.day04a)
...
(defn filter-winners [bingo-sheets]
  (filter is-winner bingo-sheets))

(defn find-winner [numbers-drawn bingo-sheets]
  ...
  (let [winners (filter-winners updated-sheets)]
  (if
    (> (count winners) 0)
    [current-number (first winners)]
    (find-winner rest-of-numbers updated-sheets))
  )))))
...
(defn do-stuff [filename]
  ...
  (let [winner-state (find-winner numbers-drawn bingo-sheets)]
  (* (first winner-state) (calculate-score (last winner-state)))
  )))))
